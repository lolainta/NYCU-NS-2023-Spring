...
class Header:
    ...
    def serialize(self):
        if self.header_form == 1:
            ...
            hdr = pack(
                f"!BLB{self.dst_id_len}sB{self.src_id_len}s8s{self.pkt_num_len+1}s",
                first_byte,
                self.version,
                self.dst_id_len,
            ...
            hdr = pack(
                f"!B{self.dst_id_len}s{self.pkt_num_len+1}s",
                first_byte,
                self.dst_id.to_bytes(self.dst_id_len, "big"),
                self.pkt_num.to_bytes(self.pkt_num_len + 1, "big"),
            )
    ...
    def deserialize(self, pkt):
        ...
        if self.header_form == 1:
            ...
            self.length, self.pkt_num = unpack(
                f"!8s{self.pkt_num_len+1}s",
                pkt[ptr + 0 : ptr + (8 + self.pkt_num_len + 1)],
            )
    ...
    def __repr__(self) -> str:
        ...
        if self.header_form == 1:
            ...
            return_str += (
                f", dst_id={self.dst_id}, src_id={self.src_id}, pkt_num={self.pkt_num}"
            )
        elif self.header_form == 0:
...
class Frame:
    ...
    def serialize(self):
        if (
            self.type == Frame.PADDING
            or self.type == Frame.PING
            or self.type == Frame.HANDSHAKE_DONE
        ):
            body = pack("!B", self.type)
        elif self.type == Frame.ACK:
            body = pack(
                f"!B{Frame.PKT_NUM_LEN+1}sBB{Frame.PKT_NUM_LEN+1}s",
                self.type,
            ...
            body = pack(
                f"!B{Frame.STREAM_ID_LEN}s{Frame.OFFSET_LEN}s{Frame.LENGTH_LEN}s",
                self.type,
                (self.stream_id | (self.directional << 62)).to_bytes(
                    Frame.STREAM_ID_LEN, "big"
                ),
            ...
            body = pack(
                f"!B8sB8s{self.reason_phrase_len}s",
                self.type,
    ...
    def deserialize(self, pkt):
        ...
        if self.type == Frame.ACK:
            (
                ...
                f"!{Frame.PKT_NUM_LEN+1}sBB{Frame.PKT_NUM_LEN+1}s", pkt[ptr + 0 :]
            )
            ...
            (stream_id, self.offset, self.length) = unpack(
                f"!{Frame.STREAM_ID_LEN}s{Frame.OFFSET_LEN}s{Frame.LENGTH_LEN}s",
                pkt[
                    ptr
                    + 0 : ptr
                    + (Frame.STREAM_ID_LEN + Frame.OFFSET_LEN + Frame.LENGTH_LEN)
                ],
            )
            ...
            (self.error_code, self.frame_type, self.reason_phrase_len) = unpack(
                "!8sB8s", pkt[ptr + 0 : ptr + (8 + 1 + 8)]
            )

            self.error_code = int.from_bytes(self.error_code, "big")
            self.reason_phrase_len = int.from_bytes(self.reason_phrase_len, "big")

            (self.reason_phrase,) = unpack(
                f"!{self.reason_phrase_len}s", pkt[ptr + 0 :]
            )
